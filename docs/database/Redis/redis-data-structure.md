...
- 最大容量：512MB（536,870,912字节）
- 说明：每个字符串键的最大长度是512MB。

2. Hash（哈希）

- 最大字段数量：2^32 - 1（大约43亿）
- 说明：一个哈希可以包含大约43亿个字段，每个字段的名字和值都是字符串。
...
- 最大元素数量：2^32 - 1（大约43亿）
- 说明：一个集合可以包含大约43亿个唯一元素。

5. Sorted Set（有序集合）

- 最大元素数量：2^32 - 1（大约43亿）
- 说明：一个有序集合可以包含大约43亿个元素，每个元素都有一个评分。

6. HyperLogLog（基数估计）

- 最大容量：12KB
- 说明：HyperLogLog的数据结构大小固定为12KB，无论它估算的集合基数有多大。

7. Bitmap（位图）

- 最大容量：512MB
- 说明：位图的最大长度与字符串相同，可以达到512MB，每个位图最多包含4,294,967,296位（即2^32位）。
